...
@RestController
public class SignApiController implements SignApi {
    ...
    public SignApiController(ObjectMapper objectMapper, HttpServletRequest request) {
    ...
    public ResponseEntity<Void> signDocument(@Parameter(in = ParameterIn.DEFAULT, description = "", schema=@Schema()) @Valid @RequestBody Document body) {
        ...
        try
        {
            String signed = sign.signDoc(body.getDocumentDir(), body.getUsername(), body.getCertificatePass(), body.getName());
            if (signed == "OK") {

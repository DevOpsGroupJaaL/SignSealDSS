...
public class CertificateApiController implements CertificateApi {
    ...
    private static final Logger log = LoggerFactory.getLogger(CertificateApiController.class);
    ...
    public ResponseEntity<Void> createCertificate(@Parameter(in = ParameterIn.DEFAULT, description = "", schema=@Schema()) @Valid @RequestBody Certificate body) {
        String accept = request.getHeader("Accept");

        try
        {
            CloudCertificates cc = new CloudCertificates();
            String out = cc.saveCertificate(body.getUsername(), body.getPassword(), body.getFullName());
            if (out == "OK") {
